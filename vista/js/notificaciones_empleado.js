...
$(document).ready(function () {
    ...
    // Nuevo evento: Manejar el clic en el botón de ir a la URL (Ver Solicitudes)
    $(document).on('click', '.btn-ir-destino', function () {
        const urlDestino = $(this).attr('href');
        const idNotificacion = $(this).data('id');
        
        // El estado de la notificación se lee del atributo 'data-estado' del botón
        if ($(this).data('estado') === 'No leída') {
            // Llama a cambiarEstadoNotificacion con los parámetros de redirección
...
function cargarNotificaciones() {
    ...
    $.post('modelo/notificaciones_empleado.php', { accion: 'listar' }, function (res) {
    ...
    datos.forEach(row => {
        ...
        switch(row.tipo_notificacion) {
            case 'Solicitud Resuelta':
                ...
                urlDestino = 'index.php?v=SolicitudesEmpleado';
                mostrarBotonIr = true;
...
function cambiarEstadoNotificacion(id, nuevoEstado, isRedirect = false, redirectUrl = '') { 
    $.post('modelo/notificaciones_empleado.php', { 
